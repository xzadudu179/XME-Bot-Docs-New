...
- **作用**
  ...
  - 全天云量：八分图，蓝色代表晴天所占的比例，白色代表云所占的比例，从左到右的云量从 0% 递增到 100%。
  - 大气视宁度：图标越小/越蓝，视宁度越好。
  - 大气透明度：横杠越少/越蓝，透明度越好。
...
- **作用**
  ...
  使用 ChatGLM 大模型的 AI 助手，主要用于解答 bot 使用方面的问题，每天可使用 30 次

  在询问 AI 的时候发送 `aistop` 可以中断 AI 对话
...
- **作用**
  ...
  在选择 `xxx吗` 的时候可能一些特殊名词会被解析为动词词组影响结果，可以使用双引号包裹特殊名词，这样就不会被解析。
...
- **作用**
  ...
  生成自己或指定群友的头像装饰环，参数可以填写以下内容：
  - maif: 旅行伙伴
  - goog: google pro 头像环
  - goog2: google pro 头像环（小头像效果）
...
- **作用**
  ...
  查看运行该 XME-Bot 实例的设备的系统状态和指令调用数据
